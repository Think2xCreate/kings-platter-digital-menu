  ...
  return (
    ...
    >
      {/* =====================================================================
          ...
          ===================================================================== */}
      <div
        className={`fixed inset-x-0 top-0 h-1/2 z-50 overflow-hidden ${
          isAdmin ? 'bg-[#09090D]' : 'bg-[#05110A]'
        } ${isExiting ? 'animate-split-top' : ''}`}
      >
        {/* Ambient royal green spotlight aura (top half) */}
        <div
          className={`absolute -bottom-28 left-1/2 -translate-x-1/2 w-80 sm:w-[460px] h-80 sm:h-[460px] rounded-full pointer-events-none filter blur-3xl ${
            isAdmin ? 'bg-[#F5B800]/12' : 'bg-emerald-600/22'
          }`}
        />
        <div
          className={`absolute -bottom-16 left-1/2 -translate-x-1/2 w-60 sm:w-80 h-60 sm:h-80 rounded-full pointer-events-none filter blur-2xl ${
            isAdmin ? 'bg-amber-600/10' : 'bg-[#E5A93C]/14'
          }`}
        />
      ...
      {/* =====================================================================
          ...
          ===================================================================== */}
      <div
        className={`fixed inset-x-0 bottom-0 h-1/2 z-50 overflow-hidden ${
          isAdmin ? 'bg-[#09090D]' : 'bg-[#05110A]'
        } ${isExiting ? 'animate-split-bottom' : ''}`}
      >
        {/* Ambient royal gold spotlight aura (bottom half) */}
        <div
          className={`absolute -top-28 left-1/2 -translate-x-1/2 w-80 sm:w-[460px] h-80 sm:h-[460px] rounded-full pointer-events-none filter blur-3xl ${
            isAdmin ? 'bg-[#F5B800]/10' : 'bg-emerald-700/16'
          }`}
        />
        <div
          className={`absolute -top-16 left-1/2 -translate-x-1/2 w-60 sm:w-80 h-60 sm:h-80 rounded-full pointer-events-none filter blur-2xl ${
            isAdmin ? 'bg-amber-600/12' : 'bg-[#E5A93C]/16'
          }`}
        />
      ...
      <div
        className={`fixed inset-0 z-50 flex flex-col items-center justify-center p-6 select-none ${
          isExiting ? 'animate-split-center pointer-events-none' : 'pointer-events-auto'
        }`}
      >
        ...
        <div className="relative z-10 flex flex-col items-center text-center max-w-sm w-full">
          ...
          <div className="relative flex items-center justify-center">
            ...
            <div
              className={`absolute -inset-3 rounded-full pointer-events-none opacity-45 filter blur-[1px] animate-spin-bezel ${
                isAdmin
            ...
            <div
              className={`relative rounded-3xl p-1.5 sm:p-2 flex items-center justify-center transition-transform animate-brand-entrance ${
                isAdmin
